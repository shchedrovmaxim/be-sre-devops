...
> Read this first. Once the analogy clicks, the [deep-dive doc](./ci-as-architecture.md) becomes obvious.
...
If that came out clearly, jump to the [deep-dive](./ci-as-architecture.md) for the DORA metrics, trunk-based development in practice, the GitOps pattern, feature flag infrastructure, and the 4-dimensions treatment.
...
When the stamp analogy, trunk-based development, and the deploy/release distinction feel obvious, jump to [`ci-as-architecture.md`](./ci-as-architecture.md). The deep-dive covers:
