...
> Read [`cgroups-memory.md`](./cgroups-memory.md) first if you haven't. The kernel/cgroup framing carries over; here we just swap memory for CPU.
...
Everything from [`cgroups-memory.md`](./cgroups-memory.md) about "cgroups are a Linux primitive, not K8s" applies identically here:
