...
> Start with the [simple version](./simple.md) if you haven't — the hotel analogy is the mental model for the detail below.
...
`pivot_root(2)` changes the root filesystem for the process. `runc` uses this (not `chroot(2)`) because `pivot_root` is safer — it can't be escaped as easily as `chroot`. More in [`runc-internals.md`](../runc-internals/deep-dive.md).
