...
> Start with [sbom-syft-simple.md](./simple.md) if you haven't. The ingredient-label analogy frames everything here.
...
The `cosign attest` step is the senior move — the SBOM is permanently attached to the image in the registry, cryptographically signed, queryable without having the original files. See [cosign-sigstore.md](../cosign-sigstore/deep-dive.md) for the full signing setup.
...
1. Signs the SBOM JSON with your signing key (or keyless OIDC — see [cosign-sigstore.md](../cosign-sigstore/deep-dive.md))
