...
> Read this first. Once the analogy clicks, the [deep-dive doc](./acme-letsencrypt.md) has the full state machine and the gotchas that surface in production.
...
3. Let's Encrypt says: "Place this specific token at `http://example.com/.well-known/acme-challenge/<token>`" (HTTP-01 challenge) or "Add this TXT record to `_acme-challenge.example.com`" (DNS-01). See [acme-challenges.md](./acme-challenges.md) for the other types.
...
When the notary-stamp analogy feels obvious and you can explain the two-key distinction without looking it up, jump to [`acme-letsencrypt.md`](./acme-letsencrypt.md). The deep-dive covers:
