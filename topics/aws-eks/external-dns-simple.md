...
> Read this first. Once the analogy clicks, the [deep-dive doc](./external-dns.md) will be obvious.
...
If that came out clearly, jump to the [deep-dive](./external-dns.md) for TXT record internals, `--domain-filter`, the multi-cluster owner ID pattern, cert-manager integration, and `--policy create-only`.
...
When the concierge analogy and the annotation-to-Route53 loop feel obvious, jump to [`external-dns.md`](./external-dns.md). The deep-dive covers:
