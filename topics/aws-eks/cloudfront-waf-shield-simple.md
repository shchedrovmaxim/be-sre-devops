...
> Read this first. Once the layers click as a castle gate, the [deep-dive doc](./cloudfront-waf-shield.md) will be obvious.
...
If that came out cleanly, jump to the [deep-dive](./cloudfront-waf-shield.md) for caching behaviors, WAF rule group pricing, Shield Advanced trade-offs, and the CF + cert-manager TLS pattern.
...
When the castle-gate layering feels obvious, jump to [`cloudfront-waf-shield.md`](./cloudfront-waf-shield.md). The deep-dive covers:
