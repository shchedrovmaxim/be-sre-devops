...
> Start with the [simple version](./simple.md) if you haven't read it. The security-guard analogy is the spine of this topic.
...
- No CPU limit intentionally — log forwarders can burst during high-write periods. CPU throttling causes log drops. Memory limit is set because OOM is less recoverable than CPU slowness. (See [resource-mgmt-requests-limits.md](../resource-mgmt-requests-limits/deep-dive.md).)
