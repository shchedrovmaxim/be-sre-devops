...
> Read this first. Once the analogy clicks, the [deep-dive](./workspaces-vs-separate-state.md) has all the detail.
...
When the master-key analogy and the IAM-isolation gap are obvious, jump to [`workspaces-vs-separate-state.md`](./workspaces-vs-separate-state.md). The deep-dive covers:
