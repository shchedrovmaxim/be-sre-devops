...
> Read this first. Once the mental model clicks, the [deep-dive doc](./secrets-comparison.md) has the full trade-off matrix.
...
When the in-Git vs runtime-pull axis is clear and you can explain the multi-cluster dealbreaker for Sealed Secrets, jump to [`secrets-comparison.md`](./secrets-comparison.md). The deep-dive covers:
